...
def generate(input_prompt, input_steps=250, input_seed=42):
  ...
  ignore_bad_gpu_error = True#@param {type:"boolean"} 
  ...
  if any(cardstr in nvidiasmi_output for cardstr in cards_requiring_downgrade) and not ignore_bad_gpu_error:    
      display(html_print("<h2 style='color: orange'>Colab gave you a GPU that can't handle default settings</h2>"))
      display(html_print("<h4>To avoid issues, either select fewer models or go to Runtime > Disconnect and delete runtime and run this cell again. You can keep doing this until you don't see this message (meaning you got a good GPU)</h4>"))
      display(html_print("<h4>If you want to proceed anyway, check `ignore_bad_gpu_error` and run all cells</h4>"))
  ...
  if is_colab:
      if google_drive is True:
          ...
          root_path = '/content/drive/MyDrive/AI/Pixel_Art_Diffusion'
      else:
          root_path = '/content'
  else:
      root_path = os.getcwd()
  ...
  diffusion_model = "PADexpanded" #@param ["pixel_art_diffusion_hard_256", "pixel_art_diffusion_soft_256", "pixelartdiffusion4k", "PADexpanded"]
  ...
  model_PADexpanded_SHA = 'a73b40556634034bf43b5a716b531b46fb1ab890634d854f5bcbbef56838739a'
  ...
  model_PADexpanded_link = 'https://huggingface.co/KaliYuga/PADexpanded/resolve/main/PADexpanded.pt'
  ...
  model_PADexpanded_path = f'{model_path}/PADexpanded.pt'
  ...
  model_PADexpanded_downloaded = False
  ...
  if diffusion_model == 'pixelartdiffusion4k':
    if os.path.exists(model_pixelartdiffusion4k_path) and check_model_SHA:
      print('Checking Pixel Art Diffusion 4k Model File')
  ...
  if diffusion_model == 'PADexpanded':
    if os.path.exists(model_pixelartdiffusion4k_path) and check_model_SHA:
      print('Checking PAD Expanded File')
      with open(model_PADexpanded_path,"rb") as f:
          bytes = f.read() 
          hash = hashlib.sha256(bytes).hexdigest();
      if hash == model_PADexpanded_SHA:
        print('PAD Expanded Model SHA matches')
        model_PADexpanded_downloaded = True
      else: 
        print("PAD Expanded 4k Model doesn't match, redownloading...")
        wget(model_PADexpanded_link, model_path)
        model_PADexpanded_downloaded = True
    elif os.path.exists(model_PADexpanded_path) and not check_model_SHA or model_pixelartdiffusion4k_downloaded == True:
      print('PAD Expanded Model already downloaded, check check_model_SHA if the file is corrupt')
    else:  
      wget(model_PADexpanded_link, model_path)
      model_PADexpanded_downloaded = True
  ...
  steps =  input_steps#@param [25,50,100,150,250,500,1000]{type: 'raw', allow-input: true}
  width_height = [475, 320]#@param{type: 'raw'}
  ...
  noise_injector = True #@param {type:"boolean"}
  ...
  set_seed = input_seed #@param{type: 'string'}
  ...
  cut_innercut ="[1]*200+[4]*200+[12]*400+[12]*200"#@param {type: 'string'}  
  ...
  cut_ic_pow = "[12]*300+[12]*100+[12]*50+[12]*50+[10]*100+[10]*100+[10]*300"#@param {type: 'string'}  
  ...
  # 
  # ```
  # ["A cyberpunk city at sunset by van gogh, #pixelart"]
  # ```
  # With a multi-part prompt, try playing around with how many times you remind it to generate images in a pixel art style. For examples of prmpt structure given varying length of prompts, see the README at the top of the page, although this may be out of date with the updated v3.1 settings. 
  # 
  # The following modifiers (and probably a lot of others) all work pretty well and give varying degrees of crunchiness:
  # - #pixelart, pixelart, pixel art
  # - #8bit, #8-bit, 8bit, 8-bit
  # - #16bit, #16-bit, 16bit, 16-bit
  # 
  # 
  # 
  # 
